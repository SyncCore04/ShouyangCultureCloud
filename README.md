...
| 文旅单位 | 单位列表、类型筛选（图书馆/文化馆/文化站/文化中心） | ✅ |
...
| 轮播图管理 | 列表/新增/编辑/删除/排序/状态 | ✅ |
| 分类管理 | 列表/新增/编辑/删除/排序/状态 | ✅ |
| 活动管理 | 列表/新增/编辑/删除/报名记录查看 | ✅ |
| 景点管理 | 列表/新增/编辑/删除/等级标签/状态 | ✅ |
| 美食管理 | 列表/新增/编辑/删除/分类标签/状态 | ✅ |
| 酒店管理 | 列表/新增/编辑/删除/星级/状态 | ✅ |
| 场馆管理 | 列表/新增/编辑/删除/容纳人数/状态 | ✅ |
| 票务管理 | 列表/新增/编辑/删除/价格/库存/状态 | ✅ |
| 非遗管理 | 列表/新增/编辑/删除/级别标签/状态 | ✅ |
| 文创管理 | 列表/新增/编辑/删除/价格/库存/状态 | ✅ |
| 展馆管理 | 列表/新增/编辑/删除/浏览量/状态 | ✅ |
| 攻略管理 | 列表/新增/编辑/删除/天数/作者/状态 | ✅ |
| 单位管理 | 列表/新增/编辑/删除/类型标签/状态 | ✅ |
...
# 第四步：启动应用（IDEA 中直接运行 ShouyangApplication.java 也可）
mvn spring-boot:run
```
...
| 前台测试用户 | testuser2 | 123456 | 前台用户端登录 |
| 前台测试用户 | test | 123456 | 旧账号（密码可能不兼容） |

> 管理员密码使用 BCrypt 加密存储，初始化脚本中已包含 admin/123456 账号。
> 项目已集成 `DataInitializer`，启动后端时会自动重置 admin 密码为 123456。
...
### Q7：为什么找不到 application-dev.yml 文件？

**A：** 为了安全，敏感的配置文件（包含数据库密码）不提交到 Git 仓库。
你需要自己从模板复制：

```bash
cd shouyang-server/src/main/resources
