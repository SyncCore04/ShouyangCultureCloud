    ...
    >
      ...
      >
        ...
        <el-form-item label="商品详情">
          <RichEditor v-model="form.content" :height="300" placeholder="请输入商品详情..." />
        </el-form-item>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await get('/admin/product/list', {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await put('/admin/product', form)
      ElMessage.success('修改成功')
    } else {
      await post('/admin/product', form)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该商品吗？删除后不可恢复。`, '确认删除', {
    confirmButtonText: '确定删除',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await del(`/admin/product/${row.id}`)
...
const handleStatusChange = async (row, val) => {
  ...
  try {
    await put('/admin/product', { id: row.id, status: newStatus })
