  ...
  <div class="guide-admin-page">
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="tableData" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column prop="title" label="攻略标题" min-width="180" show-overflow-tooltip />
    ...
    >
      ...
      >
        ...
        <el-form-item label="作者" prop="author">
          <el-input v-model="form.author" placeholder="请输入作者" maxlength="20" />
        </el-form-item>
        <el-form-item label="游玩天数">
          <el-input-number v-model="form.days" :min="1" :max="30" />
          <span style="margin-left: 8px; color: #909399; font-size: 12px">单位：天</span>
        </el-form-item>
        ...
        <el-form-item label="攻略详情">
          <RichEditor v-model="form.content" :height="300" placeholder="请输入攻略详情..." />
        </el-form-item>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await get('/admin/guide/list', {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await put('/admin/guide', form)
      ElMessage.success('修改成功')
    } else {
      await post('/admin/guide', form)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该攻略吗？删除后不可恢复。`, '确认删除', {
    confirmButtonText: '确定删除',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await del(`/admin/guide/${row.id}`)
...
const handleStatusChange = async (row, val) => {
  ...
  try {
    await put('/admin/guide', { id: row.id, status: newStatus })
