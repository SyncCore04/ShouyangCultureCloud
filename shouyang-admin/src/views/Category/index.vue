...
const loadList = async () => {
  loading.value = true
  try {
    const res = await get('/admin/category/list', {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await put('/admin/category', form)
      ElMessage.success('修改成功')
    } else {
      await post('/admin/category', form)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该分类吗？删除后不可恢复。`, '确认删除', {
    confirmButtonText: '确定删除',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await del(`/admin/category/${row.id}`)
...
const handleStatusChange = async (row, val) => {
  ...
  try {
    await put('/admin/category', { id: row.id, status: newStatus })
