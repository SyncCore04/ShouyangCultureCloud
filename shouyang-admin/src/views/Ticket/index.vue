    ...
    >
      ...
      >
        <el-form-item label="票务名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入票务名称" maxlength="50" show-word-limit />
        </el-form-item>
        ...
        <el-form-item label="价格" prop="price">
          ...
          <span style="margin-left: 8px; color: #909399; font-size: 12px">单位：元</span>
        </el-form-item>
        <el-form-item label="原价">
          <el-input-number v-model="form.originalPrice" :min="0" :precision="2" :step="1" />
          <span style="margin-left: 8px; color: #909399; font-size: 12px">单位：元，选填</span>
        </el-form-item>
        ...
        <el-form-item label="购票须知">
          <RichEditor v-model="form.content" :height="300" placeholder="请输入购票须知..." />
        </el-form-item>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await get('/admin/ticket/list', {
...
const handleEdit = (row) => {
  ...
  Object.assign(form, {
    ...
    stock: row.stock ?? row.remainCount ?? 100,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await put('/admin/ticket', form)
      ElMessage.success('修改成功')
    } else {
      await post('/admin/ticket', form)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除该票务吗？删除后不可恢复。`, '确认删除', {
    confirmButtonText: '确定删除',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await del(`/admin/ticket/${row.id}`)
...
const handleStatusChange = async (row, val) => {
  ...
  try {
    await put('/admin/ticket', { id: row.id, status: newStatus })
